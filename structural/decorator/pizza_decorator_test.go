...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the onion decorator without and IngredientAdd on its Ingredient field must reutnr an error, not a string with '%s'", onionResult)
	}
...
func TestMeat_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the meat decorator without and IngredientAdd on its Ingredient field must reutnr an error, not a string with '%s'", meatResult)
	}
	...
	if !strings.Contains(meatResult, "meat") {
		t.Errorf("When calling the add ingredient of the meat decorator it must return the text with the word 'meat', not '%s'", meatResult)
	}
}
